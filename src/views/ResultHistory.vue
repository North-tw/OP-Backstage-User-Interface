<template>
  <div>
    ...
    <!-- search -->
    <ResultHistoryListSearch
      v-model:searchState="searchState"
      @setSearchState="(isSearch) => readResultHistoryList(null, isSearch)"
    ...
    <table class="table table-hover rounded-3 overflow-hidden">
      ...
      <tbody class="border-top">
        <ResultHistoryListItem
          v-for="(item, index) in resultHistoryList"
          :key="item.id"
...
import ResultHistoryListItem from '@/components/ResultHistoryListItem'
import ResultHistoryListSearch from '@/components/ResultHistoryListSearch'
...
export default {
  ...
  async setup (props, { emit }) {
    ...
    const resultHistoryList = computed(() => queryStore.getResultHistoryList || [])

    const breadCrumbs = ref([{ title: 'Logs', class: 'd-none d-sm-inline' }, { title: 'Result History' }])
    ...
    const readResultHistoryList = async (newPage, isSearch = false) => {
      ...
      try {
        const response = await queryStore.readResultHistoryList(payload)
        // getErrorMessage
        if (!isResponseSuccess(response)) {
          setAlert(true, false, getErrorMessage(response))
        }
